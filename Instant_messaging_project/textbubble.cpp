...
bool TextBubble::eventFilter(QObject *o, QEvent *e)
{
    ...
    {
        adjustTextHeight();//painEvent中设置 自动调节高度
    }
...
{
    qreal doc_margin=m_pTextEdit->document()->documentMargin();//文本边距
    ...
    for(QTextBlock it=doc->begin();it!=doc->end();it=it.next())
    {
        ...
        text_hight+=text_rect.height();
    }
    int vMargin=this->layout()->contentsMargins().top();
    //在设置这个气泡需要的高度 文本高+文本边距+TextEdit边框到气泡边框的距离
    setFixedHeight(text_hight+doc_margin*2+vMargin*2);
}
